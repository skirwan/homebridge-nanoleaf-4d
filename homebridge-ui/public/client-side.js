...
function rgb(value, onvalue) {
  ...
  function updateColors([h, s, b]) {
    elements.hue.style.setProperty('--dynamicbackground', 
      `linear-gradient(90deg in hsl longer hue,${hsbToHslCss([0, s, b])} 0%,${hsbToHslCss([360, s, b])} 100%)`);
    elements.sat.style.setProperty('--dynamicbackground', 
      `linear-gradient(90deg in hsl longer hue,${hsbToHslCss([h, 0, b])} 0%,${hsbToHslCss([h, 100, b])} 100%)`);
    elements.brightness.style.setProperty('--dynamicbackground', 
      `linear-gradient(90deg in hsl longer hue,${hsbToHslCss([h, s, 0])} 0%,${hsbToHslCss([h, s, 100])} 100%)`);
  }
  const result = el('div', {
...
(async () => {
  homebridge.addEventListener('configChanged', showConfigurations);
