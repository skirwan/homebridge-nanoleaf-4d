...
(async () => {
    ...
    // True if we've called homebridge.updatePluginConfig at least once and the save button should be enabled
    ...
    function showConfiguration(config, scanned = false) {
        ...
        if (existing) {
            if (scanned) { return; } // Don't update if this is a scan result
